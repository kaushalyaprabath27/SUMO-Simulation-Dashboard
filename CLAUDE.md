...
| `data.js` | ~160 | Static data: 7 built-in vehicle-type defaults (all HBEFA4 `emissionClass` values as of this release — see History), param metadata, legacy `TIME_INTERVALS`. `module.exports`-guarded so `tests/` can `require()` it directly under Node. |
...
`data.js`'s vehicle-type `emissionClass` defaults were HBEFA3, and generic
for three of seven types: motorcycle, tuk_tuk, and van all shared
`HBEFA3/LDV_G_EU4`, a light-duty-vehicle class with no motorcycle-specific
factors, unreviewed and unchanged since the repository's first commit. A
referee reviewing the JSALT manuscript this tool supports caught it: the
manuscript's own verification corridor is 61.9% motorcycles, so a user who
accepted these defaults on the exact traffic mix the paper argues the tool
is for would get the dominant vehicle type modelled with car-like
emissions. Fixed to real, `emissionsMap`-verified HBEFA4 classes per type
(`MC_4S_le250cc_Euro-3` for motorcycle and, absent any HBEFA auto-rickshaw
category, tuk_tuk too; `PC_petrol_Euro-3` for passenger_car;
`LCV_diesel_N1-II_Euro-3` for van; `UBus_Std_gt15-18t_Euro-III` for
heavy_bus; `RT_gt7_5-12t_Euro-III` for truck; `HBEFA4/zero` for the
pedestrian type) — the same values an even earlier build of this tool's
UI once showed by default, before they were replaced by the current
generic HBEFA3 set at some point not captured in this repo's git history.
See `tests/vehicleTypeDefaults.test.js`.
