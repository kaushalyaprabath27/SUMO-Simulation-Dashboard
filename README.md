...
- **The idle/moving emissions split (Emissions Analysis tab) is not
  auto-generated by the "Run" button either, and needs a file the dashboard
  never asks SUMO for.** The rest of this tab reads `--tripinfo-output`,
  where each vehicle gets one `<emissions>` total for its whole trip — no
  per-timestep speed, so no way to tell idle from moving. To use the split,
  run SUMO yourself with `--emission-output` added, e.g.:
  ```
  sumo -c <your .sumocfg> \
    --tripinfo-output tripinfo.xml \
    --emission-output emission_output.xml \
    --device.emissions.probability 1.0 \
    --seed <fix one if you want this reproducible>
  ```
  and upload the resulting `emission_output.xml` there. This file is one
  `<vehicle>` row per vehicle per simulation step (not per trip), so it is
  much larger than tripinfo for the same run — a full run of this project's
  own corridor scenario produced a 308 MB `--emission-output` file against
  a 2.4 MB tripinfo file for the same 2,843 vehicles.
